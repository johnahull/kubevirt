...
func CreateDRAHostDevices(vmi *v1.VirtualMachineInstance, basePath string) ([]api.HostDevice, error) {
	...
	for _, hd := range vmi.Spec.Domain.Devices.HostDevices {
		...
		hostDevice, err := createHostDeviceForHostDevice(hd, vmi.Name, basePath, vmi.Spec)
		if err != nil {
...
func createHostDeviceForHostDevice(hd v1.HostDevice, vmiName, basePath string, vmiSpecs v1.VirtualMachineInstanceSpec) (*api.HostDevice, error) {
